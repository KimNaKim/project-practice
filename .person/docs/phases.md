...
- [ ] **Reservation 도메인:** (진행 중)
  - [ ] 고객: 예약 생성 플로우 (템플릿 완료, 백엔드 로직 미구현)
  - [ ] 관리자: 예약 현황 목록 조회, 수동 확정/취소 처리 (템플릿 완료, 백엔드 로직 미구현)
- [ ] **독립 도메인 (Notice + Gallery):** (진행 중)
  - [x] board 패키지 해체 및 분리 완료 (완료)
  - [x] 고객: 공지사항·갤러리 목록 조회 (템플릿 완료, 데이터 바인딩 미구현)
  - [ ] 관리자: 공지·갤러리 CRUD, Image 다중 업로드 (템플릿 완료, 백엔드 로직 미구현)
