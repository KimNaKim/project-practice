...
- [x] **Zone & Site 도메인:** (100% 완료)
  - [x] 고객: 메인 페이지 렌더링, 구역·사이트 목록 조회 (완료)
  - [x] 고객: 날짜별 예약 가능 사이트 필터링 (완료)
  - [x] 관리자: 구역·사이트 CRUD, 요금 설정 (완료)
- [x] **Reservation 도메인:** (100% 완료)
  - [x] 고객: 예약 생성 플로우 (신청 로직 및 요금 계산 구현 완료)
  - [x] 관리자: 예약 현황 목록 조회, 수동 확정/취소 처리 (관리자 승인/거절 기능 구현 완료)
- [x] **독립 도메인 (Notice + Gallery):** (100% 완료)
  - [x] board 패키지 해체 및 분리 완료 (완료)
  - [x] 고객: 공지사항·갤러리 목록 조회 (완료)
  - [x] 관리자: 공지·갤러리 CRUD, Image 다중 업로드 (완료)

## 💬 Phase 3: 확장 도메인 (30%) - 🟢 진행 중

**목표:** 결제 자동화, 커뮤니티, 마이페이지로 서비스 완성도 확보

> 각 도메인은 **User-facing + Admin CRUD**를 함께 구현한다.

- [ ] **Payment 도메인:** (기초 인프라 구축 완료)
  - 고객: 포트원(PortOne) V2 연동, 결제 검증, 결제 완료 시 예약 즉시 확정
  - 관리자: 결제 내역 조회
- [ ] **Qna & Comment 도메인:** (기초 인프라 구축 완료)
  - 고객: Q&A 작성
  - 관리자: 답변 등록, 답변 완료 시 수정/삭제 잠금
- [ ] **Review 도메인:** (기초 인프라 구축 완료)
